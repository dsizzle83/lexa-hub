...
// Mirrors cmd/modbus/heal_stale_test.go (this codebase's established
// convention: cmd/modbus and cmd/ocpp are separate `package main`s that
// duplicate their shared reconcile-report glue rather than share a new
// internal/ package for it — see newReconcileReportPublisher's doc).
// ---------------------------------------------------------------------
...
func TestDecideStaleHeal(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			got := decideStaleHeal(c.retained, "evse", "cs-001", now)
			if c.wantNil {
				if got != nil {
			...
			if got.DeviceClass != "evse" || got.DeviceID != "cs-001" {
				t.Errorf("DeviceClass/DeviceID = %q/%q, want evse/cs-001", got.DeviceClass, got.DeviceID)
			}
...
func TestHealStaleRetainedReportT_HealsStaleBegin(t *testing.T) {
	...
	wantTopic := bus.ReconcileReportTopic("evse", "cs-001")
	healStaleRetainedReportT(fc, "evse", "cs-001", now, time.Second, time.Second)
...
// case: no retained message on the topic at all (a healthy station with no
...
func TestHealStaleRetainedReportT_NothingRetainedIsANoop(t *testing.T) {
	...
	healStaleRetainedReportT(fc, "evse", "cs-002", time.Now(), 20*time.Millisecond, 20*time.Millisecond)
...
func TestHealStaleRetainedReportT_AlreadyEndedIsANoop(t *testing.T) {
	already := bus.ReconcileReport{
		Kind: reconcile.ReportNonConvergedEnd.String(), DeviceClass: "evse", DeviceID: "cs-003", MRID: "mrid-2",
	}
	...
	healStaleRetainedReportT(fc, "evse", "cs-003", time.Now(), time.Second, time.Second)
