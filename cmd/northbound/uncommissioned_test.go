...
// shipped factory profile (configs/factory/northbound.json, Unit 1.6) must
// be read as uncommissioned so cmd/northbound/main.go's gate takes the idle
...
func TestUncommissioned_FactoryProfile_True(t *testing.T) {
	cfg, err := loadConfig(repoPath(t, "configs", "factory", "northbound.json"))
	if err != nil {
...
// deployed example config (configs/northbound.json) has a real server and
// must be read as commissioned/configured — the gate must not idle a unit
// that is actually supposed to be walking a server.
func TestUncommissioned_BenchConfig_False(t *testing.T) {
	cfg, err := loadConfig(repoPath(t, "configs", "northbound.json"))
	if err != nil {
...
// fetcher construction (mustFetcher's log.Fatalf), exactly as it does
// today. If this ever started returning true for a missing-cert case, a
// genuinely commissioned-but-misconfigured unit would silently idle
// instead of alerting an operator to the broken cert path.
...
// cmd/northbound's test working directory (go test's cwd is the package
