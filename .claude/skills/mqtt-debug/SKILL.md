...
`ssh root@69.0.0.2` first, then mosquitto_sub locally (the listener is localhost-only).
...
| `lexa/desired/+/+` | hub issuing device desired-state docs (retained) |
| `lexa/reconcile/+/+/report` | reconciler convergence/verdict reports |
...
3. Optimizer reacted? `journalctl -u lexa-hub` + `mosquitto_sub -t 'lexa/desired/#' -v`
   (add `-t 'lexa/reconcile/#'` to watch the reconciler act on it).
